...
吃什么是基于AI美食大师的一款基于大语言模型的智能菜谱生成平台，用户可通过输入食材、菜系偏好或关键词，快速获得专业菜谱、营养分析、酒水推荐、调味方案与菜品图像。平台结合 AI 技术与中华饮食文化，支持个性化饮食推荐与创意料理生成，帮助用户提升饮食健康与烹饪体验。
...
* **小程序适配**：在现有网页端基础上，调整UI和交互方式，确保在微信小程
...
| 模块             | 功能描述                                               | 用户操作                   | 输出结果               | 优先级 |
| ---------------- | ------------------------------------------------------ | -------------------------- | ---------------------- | ------ |
| 🗓 食材保质期提醒 | 根据用户输入的食材和保质期数据，提醒用户食材的过期日期 | 用户输入食材与保质期       | 提醒用户食材即将过期   | P0     |
| 🤖 首页AI机器人   | 在首页提供智能聊天功能，解答用户关于食材、菜谱等问题   | 用户与AI机器人互动         | 展示AI生成的回答与建议 | P0     |
| 🍽️ 个性化定制餐食 | 根据用户的口味、饮食习惯等信息推荐个性化餐食           | 用户填写个人信息并选择偏好 | 个性化餐食推荐列表     | P1     |
...
**功能描述：**
 用户输入食材和相应的保质期信息后，系统自动提醒用户食材即将过期，帮助用户合理规划食材使用。
...
- 提醒信息（例如：“鸡胸肉将在3天后过期，请尽快使用。”）

**业务规则：**

- 提醒用户的时间为保质期的最后3天内
- 用户可以设置提醒的提前天数（例如：提前1天、提前2天等）
...
**功能描述：**
 在小程序首页集成AI机器人，提供即时的食材、菜谱等问题解答。用户可以通过对话与机器人互动，获取个性化的烹饪建议。

**输入参数：**

- 用户提问内容（如“今天吃什么？”“怎么做红烧肉？”）

**输出内容：**

- AI生成的回答，可能包括菜谱建议、烹饪技巧等。

**业务规则：**

- 机器人能够处理多种菜谱、食材相关问题，并返回相应的建议
- 提供上下文理解，用户可以进行连续提问
...
**功能描述：**
 根据用户的饮食习惯、口味偏好、健康需求等信息，AI提供定制化的餐食推荐。
...
- 推荐的菜品应基于用户的健康需求和口味喜好，避免过敏食材
- 系统根据季节变化和食材的时令性提供灵活调整
...
- **首页设计：**
   需要适配小程序首页，增加AI机器人和食材提醒功能。首页布局需要简洁，便于快速显示餐食推荐与保质期提醒。
- **功能页设计：**
   在现有的菜谱生成和收藏功能的基础上，增加个性化定制餐食的入口和食材保质期提醒的视图。
...
```
用户输入食材及保质期 → 系统自动记录并设置提醒 → 提前提醒用户即将过期的食材 → 用户根据提醒使用食材
```
...
```
用户打开首页 → 点击AI机器人图标 → 用户提问或与AI互动 → AI生成菜谱或解答 → 用户查看建议或菜谱
```
...
```
用户填写个人信息与偏好 → 系统生成个性化推荐菜谱 → 用户浏览推荐菜谱 → 用户选择并收藏或生成菜谱
```
...
```
{
  ...
  "dietary_preference": "low_salt",
  "favorite_ingredients": ["chicken", "tomato", "spinach"]
}
```
